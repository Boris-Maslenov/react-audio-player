...
export const AudioPlayer = () => {
    ...
    const onKeyDownHandle = (e) => {
        ...
        switch (key) {
            case 'KeyP':
                playStatus === 'pause' ? audioElement.current.play() : audioElement.current.pause();
                break;
            case 'Comma':
    ...
    return (
        <div className="audio-player-wrap" tabIndex="1" onKeyDown={onKeyDownHandle}>
            ...
            <div className="audio-player app-player__body">
                ...
                {loadStatus === 'error' ? (
                    <Error message="Ошибка загрузки media" />
                ) : (
                    <div className="audio-player__controls">
                        <div className="audio-player__buttons">
                            {playStatus === 'pause' ? (
                                <button
                                    ...
                                    className="audio-player__button audio-player__play"
                                ></button>
                            ) : (
                                <button
                                    onClick={() => audioElement.current.pause()}
                                    className="audio-player__button audio-player__pause"
                                ></button>
                            )}
                        </div>
                        ...
                        <div className="audio-player__bottom">
                            ...
                            <div className="audio-player__volume">
                                {
                                    <RangeSlider
                                        className="range-slider__volume"
                                        min={0}
                                        max={100}
                                        thumbsDisabled={[true, false]}
                                        rangeSlideDisabled={true}
